...
def fix_ar(text, for_plotly=False):
    translations = {
        ...
        "ديوانية": "ديوانية", "حمام ديوانية": "حمام", "مغاسل ضيوف": "مغاسل",
        "صالة المعيشة": "صالة المعيشة", "المطبخ": "المطبخ الرئيسي",
        "مطبخ تحضيري": "تحضيري", "غرفة غسيل": "غسيل", "غسيل علوي": "غسيل",
        "طعام": "طعام العائلة", "حمام عائلة": "حمام", "صالة علوية": "صالة علوية",
        "جناح الماستر": "جناح الماستر", "حمام الماستر": "حمام",
        "غرفة تبديل": "تبديل ملابس", "غرفة نوم 1": "نوم 1",
        "غرفة نوم 2": "نوم 2", "غرفة نوم 3": "نوم 3",
        "صالة طعام ضيوف": "طعام ضيوف", "حمام مشترك": "حمام",
        "ممر داخلي": "ممر", "بلكونة الماستر": "بلكونة"
    }
    mapped_text = text
...
def generate_blueprint_figure(rooms_data, total_area, floor_type='ground', is_green=False, prefs=None):
    ...
    for r in mapped:
        ...
        ax.text(x+w/2, y+l/2, fix_ar(r['space']), ha='center', va='center',
                fontweight='bold', fontsize=12, color='#111')
...
def create_box_plotly(x, y, z, w, l, h, color, opacity=1.0):
    x_pts = [x, x+w, x+w, x, x, x+w, x+w, x]
    y_pts = [y, y, y+l, y+l, y, y, y+l, y+l]
    z_pts = [z, z, z, z, z+h, z+h, z+h, z+h]
    i = [0, 0, 4, 4, 0, 0, 3, 3, 0, 0, 1, 1]
    j = [1, 2, 5, 6, 1, 5, 2, 6, 3, 7, 2, 6]
    k = [2, 3, 6, 7, 5, 4, 6, 7, 7, 4, 6, 5]
    mesh = go.Mesh3d(x=x_pts, y=y_pts, z=z_pts, i=i, j=j, k=k,
                     color=color, opacity=opacity, flatshading=True, hoverinfo='skip')
    ex = [x, x+w, x+w, x, x, None, x, x+w, x+w, x, x, None, x, x, None, x+w, x+w, None, x+w, x+w, None, x, x]
    ey = [y, y, y+l, y+l, y, None, y, y, y+l, y+l, y, None, y, y, None, y, y, None, y+l, y+l, None, y+l, y+l]
    ez = [z, z, z, z, z, None, z+h, z+h, z+h, z+h, z+h, None, z, z+h, None, z, z+h, None, z, z+h, None, z, z+h]
    lines = go.Scatter3d(x=ex, y=ey, z=ez, mode='lines',
                         line=dict(color='#BDC3C7', width=1.5), showlegend=False, hoverinfo='skip')
    return [mesh, lines]
...
def generate_3d_model(rooms_data, floor_type='all', is_green=False, prefs=None):
    ...
    floors_to_draw = []
    if floor_type in ['all', 'ground']:
        floors_to_draw.append(('ground', 0.0))
    if floor_type in ['all', 'first']:
        floors_to_draw.append(('first', 3.6))
    ...
    annotations = [dict(x=1.1, y=0.95, xref='paper', yref='paper',
                        text="<b>دليل الفراغات</b>", showarrow=False,
                        xanchor='right', font=dict(size=14))]
    y_pos = 0.90
    for num, name in room_legend.items():
        annotations.append(dict(x=1.1, y=y_pos, xref='paper', yref='paper',
                                text=f"<b>{num}</b> - {name}", showarrow=False, xanchor='right'))
        y_pos -= 0.05
